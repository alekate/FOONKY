...
{   
	...
	public float moveSpeed = 2500f; //4500 original
	...
	public float jumpForce = 450f; //original 550f
	private float x;
	private float y;
	private float vel;
	...
	private void Jump()
	{
        if ((grounded || wallRunning || surfing) && readyToJump)
		{
		    ...
		    if (wallRunning)
		    {
			    rb.AddForce(wallNormalVector * jumpForce * 3f); //VARIABLE PARA AJUSATAR LA FURZA MIENTRAS WALLRUN (jumpForce)
		    }
	...
	private void CounterMovement(float x, float y, Vector2 mag)
	{
		...
		float num = 0.16f; //0.16f
		float num2 = 0.01f; //0.01f;
		if (crouching)
		{
	...
	private void OnCollisionEnter(Collision hit) 
	{
		switch(hit.gameObject.tag)
		{
			...
			case"JumpBoostJumper":
			jumpForce = 1500f;
			break;

			case"Ground": //para regresar a la velocidad, salto y animmacion normal
			moveSpeed = 2500f;
			jumpForce = 450f;
			m_Animator.speed=1f;
			break;
